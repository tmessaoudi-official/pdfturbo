...
TRANSCRIPT="$TMP/transcript.jsonl"
{
  ...
  printf '%s\n' '{"type":"user","message":{"role":"user","content":"/home/user/pdfturbo is the repo root"}}'
} > "$TRANSCRIPT"

run_hook() { # $1 = stdin JSON ; sets RC and uses $TMP/out as the handoff dir
  printf '%s' "$1" | PDFTURBO_HANDOFF_DIR="$TMP/out" OBS_LOG="$TMP/obs.log" bash "$HOOK" >"$TMP/stdout" 2>"$TMP/stderr"
  RC=$?
}
...
if [[ -f "$HANDOFF" ]]; then
  ...
  if grep -qF "/home/user/pdfturbo is the repo root" "$HANDOFF"; then
...
if grep -qE '^[^#]*claude[[:space:]]+-p' "$HOOK"; then
  if grep -q 'PDFTURBO_HANDOFF_LLM' "$HOOK"; then
    ok "any LLM call is gated behind PDFTURBO_HANDOFF_LLM"
  else
...
# write, so with PDFTURBO_HANDOFF_LLM=1 a marked latest.md was still clobbered — and the log said "kept"
...
rm -rf "$TMP/out"; mkdir -p "$TMP/out"
printf 'MY HAND-WRITTEN STATE\n<!-- manual -->\n' > "$TMP/out/latest.md"
PATH="$STUB:$PATH" PDFTURBO_HANDOFF_LLM=1   run_hook "$(printf '{"transcript_path":"%s","cwd":"%s","session_id":"llm-manual"}' "$TRANSCRIPT" "$TMP")"
...
rm -rf "$TMP/out"; mkdir -p "$TMP/out"
PATH="$STUB:$PATH" PDFTURBO_HANDOFF_LLM=1   run_hook "$(printf '{"transcript_path":"%s","cwd":"%s","session_id":"llm-auto"}' "$TRANSCRIPT" "$TMP")"
