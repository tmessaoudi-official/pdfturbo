 ...
 *   their pure-JS fallbacks (row 36), and the colour-management module qcms (row 37). An explicit list, not
 *   the directory: it also holds the QuickJS scripting sandbox (never enabled here), and a pdfjs-dist upgrade
 *   that renames one of these must fail the build, not drop a decoder silently.
 * - node_modules/pdfjs-dist/iccs/<ICC_FILES> → public/pdfjs/iccs/: the default CMYK profile (row 37).
 *
 * Always recopies every file (~2.8 MB, milliseconds), so an upgrade that changes a file at the same size
...
const WASM_FILES = [
  ...
  'qcms_bg.wasm',
  // Their licences travel with them (BSD requires the notice beside a binary distribution).
  'LICENSE_JBIG2', 'LICENSE_PDFJS_JBIG2', 'LICENSE_OPENJPEG', 'LICENSE_PDFJS_OPENJPEG', 'LICENSE_QCMS', 'LICENSE_PDFJS_QCMS',
];
const ICC_FILES = ['CGATS001Compat-v2-micro.icc', 'LICENSE'];
...
const nIcc = copyAll('iccs', ICC_FILES);
console.log(`prepare-pdfjs-assets: ${nCmaps} CMap files → public/pdfjs/cmaps/, ${nWasm} wasm files → public/pdfjs/wasm/, ${nIcc} ICC files → public/pdfjs/iccs/ (licences included)`);
