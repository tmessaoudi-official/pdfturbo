 ...
 * points every getDocument call there. Idempotent: a file already present with the same size is kept;
 * pass --force to recopy. Fails loudly if the source directory is missing or the copy count differs from
 * the source count. Run via the predev / prebuild / pretest:browser npm hooks.
 */
...
for (const f of files) {
  ...
  if (!FORCE && existsSync(to) && statSync(to).size === statSync(from).size) continue;
...
console.log(`prepare-pdfjs-assets: ${files.length} CMap files in public/pdfjs/cmaps/ (${copied} copied)`);
