...
- [2026-07-28 00:20] FOUND+FIXED: six of the twelve skills still wrote to (or scanned) ephemeral `~/.claude` — `forge`, `gaps`, `handoff`, `inspect` (VISION_DIR), `retrospective`, `sweep` — while their own adaptation headers forbade it. Cause: the phorj port fixed each skill's PRIMARY report dir and missed secondary ones, and the six skills that predate phorj's own adaptation header were never adapted at all. My error was spot-checking instead of enumerating; the full-set-coverage lens exists precisely for this.
- [2026-07-28 00:20] FOUND+FIXED: `/handoff` contradicted its own header, writing to `~/.claude/projects/{slug}/memory/sessions/` and carrying two unscrubbed bundle placeholders (`/path/to/your/project`, slug `-stack`). Now `var/claude/handoff/latest.md` + a timestamped copy, matching the PreCompact hook.
- [2026-07-28 00:25] AGREED: the `npm audit --audit-level=high` gate step is TEMPORARILY disabled (developer, `e154540`), not retired, and CLAUDE.md § Git & CI now records the root cause and the restore path. Verified root cause: all 8 highs are ONE advisory (`brace-expansion` GHSA-mh99-v99m-4gvg) counted at 8 levels of a single chain under the `vite-plugin-pwa` devDependency; only `filelist/node_modules/brace-expansion@2.1.2` is actually vulnerable; `npm audit fix` ERESOLVEs because `vite-plugin-pwa@1.2.0` peer-requires vite ^3–^7 against this project's vite 8. Proposed fix is an npm `overrides` pin, which sidesteps the peer conflict.
- [2026-07-28 00:25] NOTED: CI has been red since `b815d87` — including my own bundle commits — because the audit step was still enabled and failing. The current red (`30316040594`) is different and unrelated to the bundle: `test:coverage:export` fails one test with `Failed to fetch dynamically imported module: …@pdf-lib_fontkit.js`, immediately after vite logs `Re-optimizing dependencies because vite config has changed` and `Vite unexpectedly reloaded a test`. Adding `--coverage` changes the vite config, so the dep optimiser re-runs mid-suite and the lazy `@pdf-lib/fontkit` import races the reload. The same file passes in the plain `test:browser` step. Fix belongs in `vitest.browser.config.ts` (`optimizeDeps.include`), tracked separately — it touches build config, not the bundle.
