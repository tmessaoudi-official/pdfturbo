...
- [2026-06-16] CI FIRE + FIX: master pipeline went red — `.githooks/pre-push` committed mode 100644 (core.fileMode=false → git never captured on-disk +x) → fresh CI checkout got a non-executable hook → `prePushHook.test.ts` "is executable" failed → blocked `npm run test`. Fixed `966c477` via `git update-index --chmod=+x`. (Reproduced 4 ways locally incl. Node 24.16.0 — all green — before the user supplied the failing-test name; root cause was git index mode, not code.) User dep update committed `3f36110` (validated green).
- [2026-06-16] AGREED (user "do all — challenge me"): challenged → do every VALUE item, skip explicit CEILINGs, gate M7 on real compliance triggers. Order confirmed: **#28 kill-switch FIRST** (so every new feature lands behind a flag), then M6 features (#62→#57→#61→#60→#58→#55), M5 perf (#47/#50/#49), M2 tail (#24-30), M3 #36. (AskUserQuestion)
- [2026-06-16] DONE: **#28 feature-flag/kill-switch seam** `c78f500`(seam: `src/config/features.ts` `isEnabled`, `vite-env.d.ts`, 5 tests) + `102342c`(wiring: eSign button hide / OCR searchable removal + runOcr force-visible / textEditHandler true-edit→overlay). Default ON; `VITE_FEATURE_*` env (false/0/off) or `localStorage pdfturbo.feature.<key>` disables. Full gate green (jsdom 1229+2, browser 43). 4 commits unpushed (966c477/3f36110/c78f500/102342c). NEXT: M6 #62 flattening.
...
| 28 | ✅ DONE `c78f500`(seam)+`102342c`(wiring) — **Feature-flag / kill-switch seam**: `src/config/features.ts` `isEnabled('trueEdit'/'searchableOcr'/'eSign')` reads `VITE_FEATURE_*` (default ON; false/0/off disables) + `localStorage 'pdfturbo.feature.<key>'` override; `vite-env.d.ts` types the vars. Gated: e-Sign button hidden, OCR searchable option removed + runOcr forced visible, true-edit→overlay. 5 seam tests (wiring = thin call-site conditionals). | — | S–M | absent | forge [G] |
...
| 57 | ✅ DONE — **XFDF annotation import/export**: pure codec `src/utils/xfdf.ts` (`buildXfdf`/`parseXfdf` via DOMParser, no dep) + `src/export/xfdfMapping.ts` (display↔PDF user-space flip + `pageHeightPt`). Maps highlight↔`<highlight>`, comment↔`<text>` note, text↔`<freetext>` both ways. `ExportService.exportXfdf` (XFDF↓ flyout btn) + `PDFTurboApp.importXfdf(file)` (XFDF↑ btn + hidden `xfdfInput`, one undoable MacroCmd). Gated `VITE_FEATURE_XFDF` (#28 seam). Ceiling #57b: shape/ink/stamp/line subtypes, multi-line highlight quadpoints, freetext DA appearance (fontSize is a non-standard attr for app round-trip), form `<fields>` data, rotated-page coords. Acrobat byte-exactness unverifiable here (no Acrobat); internal export→import round-trip is the guarantee. Tests: xfdf(5)+xfdfMapping(4)+xfdfExport(2)+xfdfImport(3) | M | ✅ element↔XFDF round-trip | new-ideas #6 |
