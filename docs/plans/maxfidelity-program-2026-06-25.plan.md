...
- [ ] Sub-project B build (per the spec, item-by-item, each own commit + full gate + byte-identical-when-inactive guard)
  ...
  - [ ] B1 BUILD (resume here): pure `structTreeToFlow(tree, markedItems, w, h)` — walk tree in doc order, build a marked-content-id→text map, emit H1-6→heading / P→body / L+LI→list / Table+TR+TH/TD→FlowTable grid; gate reconstructPage on struct-tree presence (skip heuristic split+assignHeadings for tagged pages); fetch `getStructTree()`+`includeMarkedContent` in exportService; tests: jsdom synthetic tree + browser real tagged PDF + byte-identical untagged control.
- [ ] Sub-project A spec → plan → build (revised A order: A5→A2→A4→TE-6a→A1→TE-6b→A6)
