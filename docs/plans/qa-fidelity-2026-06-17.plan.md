...
- [2026-06-18] DONE (TDD, UNPUSHED): **F-D D3** incremental-update multi-sign spike — **POC SUCCEEDS; the
  "structural ceiling" was WRONG.** `src/signing/incrementalSigner.ts` (EXPERIMENTAL, unwired) appends a 2nd
  independent CMS signature via a hand-built append-only incremental update (read structure w/ pdf-lib, never
  re-save; append sig dict + field + new-rev page/AcroForm + classic incremental xref/trailer `/Prev`; reuse
  byteRange.ts + buildDetachedCms). Guard `tests/signing/incrementalSigner.test.ts` (7 tests, jsdom): append-only
  prefix byte-identical, sig-1 stored ByteRange + digest unchanged, sig-2 ByteRange-from-file digest == signed
  span, pdf-lib re-parses. The ceiling was mis-attributed to pdf-lib's `save()` (renumbers objects), NOT the PDF
  format — sig-1 survives because its ByteRange ends at the original EOF. **ALREADY_SIGNED guard untouched** (Adobe
  acceptance UNVERIFIED in-repo; classic-xref + ASCII-object only). Verdict:
  `docs/reviews/2026-06-18-incremental-multisign-spike-verdict.md`. Suite 1556+2xfail, type-check+lint clean.
  **D3 reclassified: reachable (high-effort productionisation candidate), not structural ceiling.** F-D COMPLETE.
- [2026-06-18] AGREED (F-D D3 scope, AskUserQuestion): **Spike-then-verdict.** Attempt a real append-only incremental-update prototype (keep original bytes verbatim, append 2nd sig dict + field + incremental xref/trailer with /Prev, byte-splice each /Contents) — deliver a test where BOTH /ByteRange digests validate IF it works, else a precise written verdict in docs/reviews/. Keep the ALREADY_SIGNED guard until proven. 3C convergence gate run at FULL params (30/8).
