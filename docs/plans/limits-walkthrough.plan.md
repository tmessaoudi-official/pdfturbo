...
- [2026-09-26 17:55] FOUND (row 37, measured): ICC is ONE switch in pdf.js with an optional CMYK half — `useWorkerFetch` + a served `qcms_bg.wasm` turns on ICC-tagged conversion, and `iccUrl` additionally sends every DeviceCMYK colour through `CGATS001Compat-v2-micro.icc`; CMYK management cannot be had without the ICC half (`CmykICCBasedCS.isUsable` requires `IccColorSpace.isUsable`). pdf.js's own viewer sets all four URLs, so full ICC is what Firefox shows (pdf_viewer.mjs:4611-4665). Against Ghostscript renders at the same size: the ICC half changes 3 of 10 ICC-tagged corpus pages by ≤18 levels and is NOT closer to gs (6×6-block error 4.95→4.43, 2.6→2.34, 2.45→2.47); the CMYK half changes nearly every page of three government reports (≤21 levels) and IS closer — Pub 17 p1 block error 10.81→4.27 (flat pixels 13.6→0.67), rich black drawn slate-blue (44,46,53) today vs neutral (34,31,33) with it, gs (35,31,32) — with gs's default profile and with pdf.js's own alike. Controls (CJK via CMaps, base-14 fonts, plain page) pixel-identical in all configs; the viewer check stays clean with it on, +30% on the Census report (23.6→30.6 s), +4% on Pub 17, at load ~13. Probe and shots in var/claude/r37/, not committed. Not ruled.
- [2026-09-26 18:00] AGREED (row 37): turn on full pdf.js colour management — ICC-tagged AND DeviceCMYK (`iccUrl`), i.e. what pdf.js's own viewer does; serve `qcms_bg.wasm` and `CGATS001Compat-v2-micro.icc`, `useWorkerFetch` becomes true; `standardFontDataUrl` stays unset (a separate font change, not ruled).
...
- [2026-09-26 18:40] AGREED (run rules): run every ruled row, 8 to 31, in plan order, unattended; the parts only the developer can do (row 29 Acrobat checks, row 31 native Arabic review) are prepared as packs and left open.
