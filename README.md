...
- **Sanitize** — download a copy with metadata (`/Info`, XMP) and JavaScript stripped: `/OpenAction`, `/AA`, and scripts on annotations, form fields and bookmarks, including ones chained behind a legitimate link. Four kinds of non-JavaScript action go too — form submission to a URL, launching an external program, opening another document, importing form data — as do embedded files, whether attached through the document's name tree, as a **paperclip annotation**, or as an associated file on a page or annotation. Ordinary hyperlinks and in-document media actions are kept. It does **not** alter page content
