...
- **Sanitize** — download a copy with metadata (`/Info`, XMP) and JavaScript stripped: `/OpenAction`, `/AA`, and scripts on annotations, form fields and bookmarks, including ones chained behind a legitimate link. Non-JavaScript actions that reach outside the document go too — form submission to a URL, launching an external program, opening another document, importing form data — as do embedded files, whether attached through the document's name tree or as a **paperclip annotation**. Ordinary hyperlinks are kept. It does **not** alter page content
