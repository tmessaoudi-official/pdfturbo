...
- **Sanitize** — download a copy with metadata (`/Info`, XMP) and JavaScript stripped: `/OpenAction`, `/AA`, and scripts on annotations, form fields and bookmarks, including ones chained behind a legitimate link. Files embedded through the document's name tree go too; a file attached as a **paperclip annotation** currently stays — see [KNOWN_ISSUES.md](KNOWN_ISSUES.md). It does **not** alter page content
