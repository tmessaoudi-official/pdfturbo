...
- **Lock PDF** — AES-256 password encryption on export; a few kinds of string cannot be encrypted, listed in [SECURITY.md](SECURITY.md#lock-pdf--what-the-password-encrypts-and-what-it-cannot)
- **Sanitize** — download a copy with metadata (`/Info`, XMP on every object, private application data) and JavaScript stripped: `/OpenAction`, `/AA` wherever it appears (including where a reader would inherit it from a parent), scripts on annotations, form fields and bookmarks, including ones chained behind a legitimate link, and 3D artwork scripts. Four kinds of non-JavaScript action go too — form submission to a URL, launching an external program, opening another document, importing form data — as do embedded files, whether attached through the document's name tree, as a **paperclip annotation** (on a page or hidden inside a form field), or as an associated file on any object; the file's bytes leave the copy even when something else in the document still pointed at them. Ordinary hyperlinks and in-document media actions are kept. It does **not** alter page content
