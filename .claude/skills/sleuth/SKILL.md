...
## pdfturbo lens K — MANDATORY additional agent (output-path divergence)
...
```bash
ls "$TARGET"/{package.json,Cargo.toml,pyproject.toml,go.mod,Makefile,docker-compose*.yaml,*.sh} 2>/dev/null
...
Summarize: tech stack, project type, primary language(s). Pass as `PROJECT_CONTEXT` to each agent.
...
Replace `<TARGET>` with the actual path. Replace `PROJECT_CONTEXT`, `CURRENT_DATE`, `PRIOR_SLEUTH_NOTE`. When constructing each agent prompt, replace `$OUTPUT_FILE` with `$SLEUTH_DIR/raw/A.md` for Agent A, `$SLEUTH_DIR/raw/B.md` for Agent B, … `$SLEUTH_DIR/raw/J.md` for Agent J.
...
> ### [High|Medium|Low] — [Short title] — Category [A-J]
> **Location**: `path/to/file:line`
> **Type**: [Logic Trap / Silent Failure / Contract Violation / Cross-Component / Edge Case / Shell Bug / Config Bug / Timing Bug / Container Bug / Doc Divergence]
...
>
> **Missing Coverage** *(behavioral bug categories with no agent at all in A–J)*
...
**Close with a non-blocking offer** (pdfturbo adaptation — no interrupts). State plainly: `N findings
